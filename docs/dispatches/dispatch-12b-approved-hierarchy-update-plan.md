...
## Delivery review — focused corrections to the plan at 2929ccf

The approved business mapping is unchanged. The 50-row manifest contains 50 unique Location IDs and the correct 14/12/22/2 totals. Do not ask Theo to reapprove those assignments. The following evidence and execution-detail corrections are needed before calling this an exact, executable plan.

1. **Saved values versus calculated defaults.** The manifest's CurrentHierarchyApplicability currently contains effective values, hiding that 49 records have no saved value. Regenerate from a fresh read with saved-value presence, raw saved value, and effective value in separate columns. Represent missing/null/empty distinctly for every proposed field. Do not reconstruct missingness from the existing manifest's effective values. Compute ChangedFields from actual before/after differences: Store 150 should list only districtId when its existing applicability and Region still match. Registry-resolved DistrictName is a derived result, never a Location field to persist. Add manifest validation that detects these two defects and confirms every approved field and preserved field.

2. **Freeze one batch plan and publish checksums now.** Remove the competing batching alternatives. For the planning manifest, use Batch 1 = the first 40 approved retail Locations sorted lexicographically by exact document ID; Batch 2 = the remaining 8 retail Locations followed by the two centers sorted by exact document ID. Record BatchId and all members in the manifest/plan, and publish the manifest SHA-256. Any drift must be listed and reconciled before using those counts; do not force new or changed records into the approved scope. Checksums are agent-generated verification evidence, not a technical decision Theo must calculate.

3. **Demonstrate the proposed path and state recovery limits accurately.** Select and document the exact existing authorized update-only execution path after checking that all forward fields are supported. Validate the proposed payloads using local fixtures/read-only preparation without requesting live confirmation or generating writes. Identify validator blockers rather than silently selecting an unspecified equivalent path. Existing receipts are evidence, not a restore mechanism. Show the supported restore operation for each changed field, including absent applicability, removed hierarchy references, and loc-86's prior empty name/type. Restoring invalid DIS-01 may be rejected and must not be promised. If exact restore is unsupported, mark that limitation explicitly and propose a bounded compensating recovery for later approval; do not bypass validation or add apply/recovery code in this pass.

4. **Provenance and test evidence.** The plan repeats the earlier 12A read interval while calling it a fresh 12B read; tie the manifest to the actual reader invocation/source, read interval, counts, and drift comparison. If fresh evidence was not retained, perform one authorized read-only refresh and report it honestly. Reconcile the committed 12A NOT RUN entries with the latest verifiable results. The pasted command counted lines starting with a checkmark, which is not proof of the total test count; use the test runner's final tests/pass/fail summary and captured exit status. Record command, tested source, counts, and evidence path. Reuse existing logs if sufficient; avoid redundant full reruns. Correct the report rather than claiming 171 is verified by that grep.

5. **Finish the concrete cross-surface proposal.** Recommend one additive contract for review: API hierarchyApplicability exposes the effective enum with the existing missing-value defaults; reporting CSV uses the matching HierarchyApplicability heading; saved value/presence stays available in internal review evidence. Editing exports retain the saved-value semantics needed for unchanged round trips. Explain and test any reporting-export re-import difference rather than silently converting defaults to saved changes. Preserve existing hierarchyStatus values, IDs/names, legacy fields, permissions, and contact privacy. State exactly how PDF/UI use applicability for Operational Centers versus Unassigned Retail Locations without discarding invalid-reference warnings. The implementation remains a later approved task.

6. **Separate blockers from follow-up.** Provide a clear recommended implementation/deployment/data-update order and a concrete restricted-storage proposal grounded in available infrastructure, without creating resources. If the storage cannot be verified, state that specific gap. External consumers remain NOT VERIFIED, but inventory uncertainty alone must not be presented as proof the approved additive mapping cannot be prepared. Breaking compatibility removal remains deferred. Present only unresolved business/access decisions to Theo; select routine technical details in the plan.

Update the existing 12B report and manifest, with focused read-only helper/tests only if needed. Preserve the historical 12A snapshot and add a correctly dated evidence/status follow-up. Validate exact field differences, approved identity-to-District mapping, batch membership/checksum, no writes, and source provenance; follow the existing executable-change test requirements if helper code changes.

Commit and push the scoped correction and link the evidence in PR #10. Preserve the unrelated runbook edit. No changes to approved business assignments, runtime application, live data, imports, repairs, migrations, deployments, merges, external messages, or browser automation.
