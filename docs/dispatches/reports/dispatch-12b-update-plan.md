...
- Fresh read: corrected helper read `locations`, `regions`, and `districts` in one read-only Firestore transaction. Wall-clock interval was `2026-09-21T02:19:07.569Z` to `2026-09-21T02:19:09.002Z`; this is evidence of the read operation, not a database generation timestamp.
...
- The companion CSV is review-only and **not import-ready**. It contains one field-level row per Location, including unchanged values and raw version presence.

## Exact proposed data changes

The complete field-level manifest is [dispatch-12b-proposed-changes.csv](dispatch-12b-proposed-changes.csv). It is the only source for a later approved payload; no row may be added from manager names, geography, legacy text, or a refreshed screenshot.

### Retail stores

For all 48 approved retail stores, set `hierarchyApplicability` explicitly to `Applicable`, set `regionId` to exact string `region-west`, and set `districtId` to the approved exact string `01`, `02`, or `03`. Preserve every other field. Store 150 changes only from invalid `DIS-01` to approved `02` for the canonical District reference; its legacy `district` text remains unchanged for compatibility.
...
4. The existing confirmation limit is 40 changed rows. Use deterministic batches: Batch 1 = the first 40 manifest rows in canonical Location ID order; Batch 2 = the remaining 10 rows. A safer operational split is 40 retail rows, then 8 retail rows plus the 2 centers, but the exact membership must be frozen and recorded in the approved execution receipt. Re-preview and re-confirm each batch against a fresh read. This is not one atomic transaction across all 50 rows.
...
Recovery is a new approved write plan, not traffic rollback. For each completed batch, compare the captured before-state with the current document and require the current version to be the exact version produced by the approved batch. Restore only the approved business fields that this plan changed, using a new expected-version write and new audit/version history. Do not reset versions or timestamps and do not overwrite later legitimate edits. If a document has changed after the batch, stop and obtain a row-specific recovery decision.

For Store 86, recovery may restore its prior blank name/type and applicability only when the current version gate passes; its legacy District is not changed. For missing fields, restoration must use the application's explicit clear semantics rather than omitting the field. Historical invalid references are preserved as evidence and are not blindly restored as a canonical relationship. Existing CSV confirmation receipts/audits are the approved recovery evidence mechanism; no new apply-capable helper is proposed here.
...
| Reporting export | `server/locationExport.ts` emits Region/District IDs/names and legacy District but omits applicability. | Additive future applicability projection must distinguish centers from unassigned retail; preserve legacy fields and existing meanings. Test 48 retail + 2 centers and scoped totals. | External reporting consumers NOT VERIFIED. |
...
1. The exact approved manifest checksum and deterministic batch membership before execution.
2. Whether the additive `hierarchyApplicability` API/report/PDF/UI field is approved and whether saved and effective values are both exposed.
3. The external owners and verification evidence for direct database readers/writers, scorecards, scheduled jobs, integrations, and PDF consumers.
4. The final execution/deployment order for the later runtime correction versus data update.
5. The recovery approver and restricted storage location for before/after evidence.
6. The later compatibility transition for Store 86's preserved legacy District text and the Person/Fleet legacy writers.
