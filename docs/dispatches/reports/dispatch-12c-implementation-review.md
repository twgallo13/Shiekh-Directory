...
## Final correction — unclassified type must precede the District-ID early return (review of `25be0ffc`)

An independent review ran eight synthetic cases against the committed helper at application SHA `25be0ffc3ec796c2d1f23aca7acc736b5142d4ea` and confirmed all eight already-corrected cases pass. It found one remaining defect: both `resolveHierarchyGroupKeyForClass` and `hierarchyDistrictLabel` checked `hierarchy.districtId` and returned through the ordinary District branch **before** considering type classification. So `{ type: 'Unsupported Type', regionId: 'west', districtId: '01' }` against an Active District `01` resolved to group `district:01` and label `District One (01)` with no type warning at all — an unclassified type was silently certified as a normal, healthy District assignment merely because a District happened to be present.

**Fixed:** the unclassified-type check now runs before the District-ID branch in both functions. An unclassified type (missing or unrecognized) always resolves to group `needs-review`, regardless of whether a District is assigned, and `hierarchyDistrictLabel` always leads with `Needs Review - Location type is '<type>'/missing, not a recognized business type.` Critically, the original District identity and every hierarchy reference diagnostic are still preserved and appended to that label — nothing is erased: a valid assigned District shows its resolved name and ID (e.g. `District One (01)`) after the type diagnostic; an unresolved, retired, or parent-mismatched reference still shows its exact diagnostic text (e.g. `District missing is missing from the hierarchy registry.`, `District 01 is retired.`, `District 01 belongs to Region west, not east.`) alongside the type diagnostic. `hierarchyStatus` and the public API shape are unchanged; known business types (retail and recognized non-retail) are unaffected and retain their existing stable District-ID grouping.

Changed files: `src/lib/hierarchyResolution.ts` (reordered the unclassified check ahead of the District-ID branch in `resolveHierarchyGroupKeyForClass` and `hierarchyDistrictLabel`; `needsReviewReason` now accepts an optional District-identity context so it can be appended even when the District branch is bypassed) and `test/hierarchyResolution.test.ts` (one pre-existing assertion that exercised this exact bug via a bare `{ districtId: 'missing' }` fixture with no `type` was corrected to pass an explicit known type, since it was unintentionally asserting the old, buggy no-warning behavior).

### New focused regression

A single parameterized test covers both a missing type and an unsupported type across the three required scenarios: (a) no references — Needs Review; (b) a valid, correctly parented, Active assigned District — Needs Review with the resolved District name/ID preserved in the label; (c) an unresolved reference — Needs Review with both the exact reference diagnostic and the unresolved District ID preserved. It also separately covers a retired-District reference and a parent-mismatched reference for an unclassified type, and confirms a known business type with an assigned District is unaffected (still plain stable District-ID grouping/label). All eight previously corrected cases were re-run and continue to pass.

## Verification

- `npm run lint` (tsc --noEmit): **PASS**, exit 0.
- `npm run test:api`: **PASS** — final test-runner summary: `tests 277, pass 277, fail 0, cancelled 0, skipped 0`.
