...
export function AccountView() {
  ...
  return <section className="max-w-3xl space-y-6 text-neutral-900">
    ...
    {account.authenticationMethod === "password" && <div className="space-y-3 border-t border-neutral-200 pt-5 text-sm"><p>To change your password, request a secure reset email. Email-link users can continue signing in by email link.</p><button type="button" disabled={busy || !account.email} className="flex items-center gap-2 rounded-md border border-neutral-300 px-3 py-2 disabled:opacity-50" onClick={async () => { setBusy(true); try { await authActions().reset(account.email!); setMessage("If this account is eligible, a password-reset email will arrive shortly."); } catch (error) { setMessage(authErrorMessage(error)); } finally { setBusy(false); } }}><KeyRound className="h-4 w-4" />Send password reset</button></div>}
