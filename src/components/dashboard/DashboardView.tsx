...
export const DashboardView: React.FC<DashboardViewProps> = ({
  ...
  const districtMap = useMemo(() => {
    ...
    locations.forEach(l => {
      ...
      const key = resolveHierarchyGroupKey(hierarchy, l.type);
  ...
  const quickRefStores = useMemo(() => {
    return locations.filter(l => {
      const hierarchy = hierarchyByLocationId.get(l.id);
      const matchesDistrict = selectedDistrict === 'all' || hierarchyGroupId(resolveHierarchyGroupKey(hierarchy, l.type)) === selectedDistrict;
                  ...
                  <div className="mt-2 pt-2 border-t border-neutral-200/70 flex items-center justify-between text-[10px] text-neutral-500">
                    <div className="flex items-center gap-1">
                      ...
                      <span className="font-semibold text-neutral-700">
                        {hierarchyDistrictLabel(hierarchy, loc.type)}
                      </span>
                      {dmPerson && (
                        <button
                          type="button"
